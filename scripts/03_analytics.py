...
def delisting_paths(detail):
    """组合持有过、后来真的退市了的股票，究竟是从哪扇门离开组合的。

    「退市 0 次」这个结果本身需要解释，否则看上去像是分类漏了一档。实际情况
    是：濒临退市的股票会先停牌、先戴帽，本文的过滤链在那时就已经把它挤出去
    了，等到退市那天它早就不在组合里。研报统计到的 2 次退市，用的必然是另一
    种口径——按股票**后来的命运**归因，而不是按调出当天的状态。
    """
    ...
    print(f"  退市逐年：全时段 0 次 —— 研报 2 次（0.08%）。这个 0 是有原因的：")
    print(f"  组合持有过的股票里有 {len(touched)} 只后来真的退市了，但**没有一只**是在退市"
          f"当天被调出的。")
    print(f"  它们最后一次离开组合的原因：{dict(rows['reason'].value_counts())}，"
          f"离退市还有 {lead.median():.0f} 天（中位数，最短 {lead.min():.0f} 天）。")
    print(f"  ⇒ 濒临退市的股票总是先停牌或先戴帽，过滤链在那时就已经把它挤出去了。"
          f"研报的「退市」必然是按股票**后来的命运**归因，不是按调出当天的状态。")
